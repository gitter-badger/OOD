...
let ProviderClass = class Provider {
  ...
  HTTPBasicAuth(path, serviceName) {
      path = (serviceName) ? path + serviceName + ".auth" : path + "/" + "folder.auth";
      if(__fs.existsSync(path)) {
        var auth = this.request.headers['authorization'];
        if(auth) {

          let authFileContent = __fs.readFileSync(path,"utf8");
          let tmp = auth.split(' ');
          let buf = new Buffer(tmp[1], 'base64');
          let plain_auth = buf.toString();
          plain_auth = __utilities.class.parseAuthStringToMD5(plain_auth);
          var foundedUser = false;

          for(let userString of authFileContent.split("\n")) {
            if(plain_auth == userString.trim()) {
              foundedUser = true;
            }
          }

          if(!foundedUser) {
            this.responds.statusCode = 401;
            this.responds.setHeader('WWW-Authenticate', 'Basic realm="Secure Area"');
            this.responds.end('<html><body>Ths is a Secure Area</body></html>');
          }
          ...
          this.responds.statusCode = 401;
          this.responds.setHeader('WWW-Authenticate', 'Basic realm="Secure Area"');
          this.responds.end('<html><body>Need some creds son</body></html>');
          return false;
        }
      } else {
        return true;
  ...
  start() {
    ...
    __http.createServer((req, res) => {
      ...
      ***/
      ...
      //sesion
      providerInstance.sessionManager.loadSession(providerInstance);
      ...
      if(__fs.existsSync(filePath) || isFile) {
          ...
          let auth = providerInstance.HTTPBasicAuth((isFile) ? providerInstance.config.resourcesPath + __utilities.class.fetchFolderPath(providerInstance.urlObject.components) : filePath);
          if(auth) {
            if(!isFile) {
        ...
        let serviceName = providerInstance.urlObject.components[0];
        var servicePath = providerInstance.config.servicesPath + serviceName + ".js";
        let auth = providerInstance.HTTPBasicAuth(providerInstance.config.servicesPath,serviceName);
        if(auth) {
          providerInstance.saveOperationHandler(() => {
